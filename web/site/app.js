// The player: enter a puzzle, hand it to the solver running on this page, then replay the steps.
// State is recomputed from the givens up to the current step - the solver's steps are deltas, and
// its own tests verify that applying them reproduces the solution. The solver itself is solver.js,
// the Kotlin library compiled to JavaScript; it registers `sudokuSolver` before this file runs.
'use strict';

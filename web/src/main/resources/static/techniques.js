...
const TECHNIQUES = {
  FULL_HOUSE: {
    ...
    text: 'A house — a row, column or box — with exactly one unsolved cell. Eight of its nine ' +
      'values are already placed, so the gap must take the ninth. The cheapest check in the game, ' +
      'and the first thing a human looks for.',
    example: 'row:  4  1  7  9  {6}  2  8  5  3      ->  the gap must be 6',
  },
  NAKED_SINGLE: {
    name: 'Naked single',
    level: 'Level 1 — Singles',
    text: 'A cell down to a single candidate: every other value already appears somewhere in its ' +
      'row, column or box. Nothing is left to decide — the cell is that value. Together with the ' +
      'other singles, this is where digits actually get placed; every fancier technique below ' +
      'exists only to create work for this level.',
    example: '{5}  ->  5',
  },
  HIDDEN_SINGLE: {
    name: 'Hidden single',
    ...
    text: 'A value that fits in only one cell of a house, even though that cell still has other ' +
      'candidates. The single is “hidden” behind them — you find it by asking where the VALUE can ' +
      'go, not what the CELL can be. This is what crosshatching finds, and it carries most easy ' +
      'puzzles on its own.',
  ...
  PEER_ELIMINATION: {
    ...
    text: 'The bookkeeping behind everything else: a value placed in a cell cannot appear again ' +
      'in that cell’s row, column or box, so it is crossed off the pencil marks of the 20 ' +
      'cells it sees. Not a trick — just the rules of the game applied — but every technique ' +
      'above reads the pencil marks this maintains.',
  },
  POINTING: {
    ...
    text: 'Inside one box, every place a value can still go lies on a single row or column. The ' +
      'box must contain that value somewhere, so the value is pinned to that overlap — and it can ' +
      'be crossed off everywhere else along the line. Two rules like this one carry most ' +
  ...
  CLAIMING: {
    ...
    text: 'The mirror image of pointing. Along one row or column, every place a value can go ' +
      ...
      'copy — so the value is crossed off every other cell of the box.',
  ...
  NAKED_SUBSET: {
    ...
    text: 'k cells of one house whose candidates, taken together, come to exactly k values. ' +
      'Those cells use those values up between them — no cell needs to hold all of them — so the ' +
      'values vanish from every other cell of the house. Watch what it leaves behind: collapsing ' +
      'one cell to a single candidate hands the grid straight back to the singles.',
  ...
  XY_WING: {
    ...
    text: 'Three cells with two candidates each: a pivot {x,y}, and two pincers {x,z} and {y,z} ' +
      ...
      'pincer and forces it onto z. So one of the pincers is z — you cannot say which, and you ' +
      'don’t need to: any cell that sees both pincers cannot be z.',
  ...
  XYZ_WING: {
    ...
    text: 'Like the XY-Wing, but the pivot keeps the shared value too: pivot {x,y,z}, pincers ' +
      '{x,z} and {y,z}. The three cells must share out the three values, so one of the THREE is ' +
      'z. That weakens the conclusion: only a cell that sees all three of them can drop z.',
  },
  W_WING: {
    name: 'W-Wing',
    level: 'Level 5 — Wings',
  ...
  EMPTY_RECTANGLE: {
    ...
    text: 'Inside one box, every place for a value fits on one row plus one column, leaving the ' +
      'opposite 2×2 corner empty. That cross shape is itself a strong statement: the ' +
      'box’s value is on the row arm or the column arm. Combined with a conjugate pair ' +
      'elsewhere, it pins a cell that would push the value off both arms at once.',
  },
  UNIQUE_RECTANGLE: {
  ...
  BUG_PLUS_ONE: {
    name: 'BUG+1',
    level: 'Level 6 — Uniqueness',
    text: 'BUG: a bivalue universal grave — every unsolved cell a pair, every value left with ' +
      'exactly two homes per house. Such a grid always has an even number of solutions, so a ' +
      'proper puzzle can never reach one. When a single cell stands between the grid and a grave, ' +
      'that cell must hold the one candidate whose removal would dig it.',
  },
  AIC: {
    ...
    text: 'The general form of chain reasoning, over (cell, value) nodes. A strong link means ' +
      '“if one end is false, the other is true”; a weak link the reverse. A chain that ' +
      'alternates strong, weak, strong, …, strong proves that one of its two ENDS is true — ' +
      'follow it: if the first node is false the second is true, so the third is false, and so on ' +
      'to the last. Both ends the same value: cells seeing both drop it. Both ends in one cell: ' +
      'that cell holds one of the two, and its other candidates go.',
  ...
  FINNED_FISH: {
    ...
    text: 'A fish spoiled by extra candidates — fins — in one of its base lines. Split the ' +
  ...
  ALS_XZ: {
    ...
    text: 'An almost locked set is n cells of a house holding n+1 values — one short of locked, ' +
      ...
      'every value it holds — so their other shared value Z sits inside one of them for certain.',
  },
  GUESS: {
    ...
    text: 'Every proper puzzle is fully deducible in principle, but no fixed set of techniques ' +
      ...
      'contradiction. That is search, and this site refuses to dress it up as deduction: what you ' +
      'see marked as a guess IS a guess. Branches that failed are not shown as if they were ' +
      'reasoning — they were not.',
  },
};
